module.exports = function () {
  return {
    ...
    writeToReportSomething(data, field) {
      ...
      } else if (field === this.reportUtil.jsonNames.test) json.fixtures.find(fixt => fixt.name === console.currentFixtureName).tests.push(data);else if (field) json[field] = data;
    ...
    setTestStatus(status) {
      ...
      if (currentStatus !== this.testStatuses.broken) this.setLastTestProperties({
    ...
    reportTaskStart(startTime, userAgents, testsCount) {
      try {
        ...
        if (!this.fs.existsSync(this.reportUtil.getReportPath())) this.fs.mkdirSync(this.reportUtil.getReportPath(), {
          recursive: true
        });
    ...
    reportTestDone(name, testRunInfo) {
      try {
        ...
        for (const error of stackTrace) {
          for (let index = 0; index < error.length; index++) console.log(this.chalk.rgb(...this.chalkStyles.stackTrace)(' '.repeat(index) + error[index]));
        }
    ...
    reportTaskDone(endTime, passed, warnings) {
      try {
        ...
        if (passed !== this.testsCount) {
          summary += ', ' + this.chalk[this.chalkStyles.failed](`${this.testsCount - passed - this.skippedCount}/${this.testsCount} ${this.testStatuses.failed}`) + ', ' + this.chalk[this.chalkStyles.skipped](`${this.skippedCount ? this.skippedCount : 0} ${this.testStatuses.skipped}`);
        }
